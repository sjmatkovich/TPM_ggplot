...
types <- read.table(file="GSE116250_unpaired_design-disease-gender-age-ethn.txt", row.names=1, header=TRUE, stringsAsFactors = FALSE) # sample grouping by disease
lengths <- read.csv(file="GRCh38-94_ENSG_lengths.csv", row.names=1)
...
## process voom_log2cpm table so that TPM can be adjusted to correspond to linear modelling statistical results
      ...
      trans.voom.adj <- mapply('/', select(trans.voom.types,-c("samples","disease")), voom.group.means[["NF"]])
